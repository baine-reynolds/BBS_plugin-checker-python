...
class Formatter:
	def format(disabled_bundled, raw_apps, env, markdown):
		...
		if markdown:
			...
			if len(not_compatible_latest) > 0:
				print(f"\nThe following Plugins are installed but NOT COMPATIBLE with your current version of {env.product}: {env.version}, however, there are no newer releases." +
					" We recommend disabling these plugins and finding alternatives.")
				for app in not_compatible_latest:
					print(Fore.RED + f"  Name:{app[2]}   Key:{app[1]}   Current Version:{app[3]}" + Style.RESET_ALL)
			if len(not_latest) > 0:
